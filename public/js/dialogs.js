...
setDialogClose(); // Renders lower content of dialog (this usually contains some sort of action)

function renderDialogActions(id) {
  var dialogActions = $("<div/>").addClass('dialogActionsContainer'); // Clear Save Dialog
  ...
  if (id === 'playfab') {
    ...
    $("<div/>").addClass("cloudSaveLogo").prependTo(dialogActions); // Login or Registration Form
    ...
    $("<div/>").attr({
      id: "pfStatus"
    }).addClass("pfStatus").appendTo(statusMessage); // Importing or exporting from Cloud
    ...
    $("<div/>").attr({
      id: "pfStatusSave"
    }).addClass("pfStatusSave").appendTo(accountImportExport); // Confirmation screen for loading from Cloud
  ...
  if (id === 'settings') {
    var settingsContainer = $("<div/>").attr({
      id: "settingsGrid"
    }).addClass("settings-grid").appendTo(dialogActions); // Setting: Notification Location
    ...
    $("<button/>").attr({
      id: "clearSettings"
    }).addClass("actionButton").html("Reset All Settings").appendTo(clearSettings); // Return Value
...
function setDialogOpen(props) {
  ...
  if (props.id === 'patch_notes') $(dialogBoxContainer).addClass('isPatchDialog'); // Dialog Upper Content
  ...
  if (props.id === 'patch_notes') showPatchNotes(); // Settings update
