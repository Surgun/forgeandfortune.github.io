...
var DungeonSeedManager = {
  getFloorSeed: function getFloorSeed(dungeonID, floor) {
    if (this[dungeonID] === undefined) this[dungeonID] = [DungeonManager.dungeonByID(dungeonID).seed];
    var dungeonSeedList = this[dungeonID];

    while (dungeonSeedList.length < floor) {
      var num = dungeonSeedList[dungeonSeedList.length - 1];
