    ...
    refreshBuff() {
        if (DungeonManager.dungeonView !== this.dungeon) {
            ...
            this.allies.forEach(ally => {
                const $heroDiv = $("#heroBuff"+ally.id);
                $heroDiv.empty();
                ally.buffs.forEach(buff => {
                    const d1 = $("<div/>").addClass("buffContainer").attr("id",ally.id+buff.id).appendTo($heroDiv);
                        $("<div/>").addClass("buffContainerIcon").html(buff.icon).appendTo(d1);
                        const count = (buff.onCast === "expire") ? buff.turns : buff.stacks;
                        $("<div/>").addClass("buffContainerCount").attr("id","count"+ally.id+buff.id).html(count).appendTo(d1);
                });
            });
            this.enemies.forEach(enemy => {
                const $enemyDiv = $("#enemyBuff"+enemy.id);
                $enemyDiv.empty();
                enemy.buffs.forEach(buff => {
                    const d2 = $("<div/>").addClass("buffContainer").attr("id",enemy.id+buff.id).appendTo($enemyDiv);
                        $("<div/>").addClass("buffContainerIcon").html(buff.icon).appendTo(d1);
                        const count = (buff.onCast === "expire") ? buff.turns : buff.stacks;
                        $("<div/>").addClass("buffContainerCount").attr("id","count"+enemy.id+buff.id).html(count).appendTo(d1);
                })
            })
        }
    }
