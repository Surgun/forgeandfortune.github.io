...
function refreshHallMonsterInspect(monster) {
    ...
    const stats = [`${monster.getHP(floorRange.min)} - ${monster.getHP(floorRange.max)}`,`${monster.getPow(floorRange.min)} - ${monster.getPow(floorRange.max)}`, monster.spow, monster.armor, monster.crit+"%", monster.dodge+"%"];
    ...
    ItemType.forEach(type => {
        ...
        const d3 = $("<div/>").addClass("lineRewardPay").attr("id","monsterPay").data("line",type).appendTo(d);
            if (MonsterHall.maxUpgrade(type)) {
                ...
                $("<div/>").addClass("lineRewardPayCost tooltip").attr({"data-tooltip":"material_desc","data-tooltip-value":"M002"}).html(`1 ${miscIcons.trophy}`).appendTo(d3);
            }
    });
}
